...
A PCAP-first platform for two-stage anomaly detection in IT and OT traffic. It supports SSH, DNS, HTTP, Modbus TCP, and S7comm, with two deployment strategies:

- `per_protocol`: one detector per protocol.
- `grouped`: one detector for the selected IT protocols or OT protocols.

The current workflow provides audit-grade PCAP/CSV mapping, protocol-separated feature files, immutable feature profiles, preprocessing comparison, live resource monitoring, and a deployable LSTM-AE + Isolation Forest to Random Forest pipeline. Models are saved in their native format and ONNX.

## Recommended workflow (current dataset layout)

```text
data/raw/benign/pcap/<protocol>/*.pcap|*.pcapng
data/raw/attack/pcap/<protocol>/*.pcap|*.pcapng
data/raw/attack/labels/<protocol>/*.csv
```
