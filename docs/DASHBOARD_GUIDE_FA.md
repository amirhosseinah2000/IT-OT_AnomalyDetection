...
| `Maximum source rows for models` | سقف رکوردهای ورودی برای تمام مدل‌های اجرای تعاملی. نمونه‌گیری به‌صورت پخش‌شده در طول فایل انجام می‌شود تا داشبورد حافظهٔ کل جدول‌های چندمیلیونی را مصرف نکند. برای آموزش کامل آفلاین، مقدار `models.max_source_rows` را فقط با RAM کافی `null` کنید. |
