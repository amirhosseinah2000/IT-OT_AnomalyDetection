...
def test_dashboard_prefers_nested_pipeline_runs_over_legacy_root_table(tmp_path) -> None:
    """A legacy direct table must not appear beside actual pipeline run directories."""
    artifact_root = tmp_path / "artifacts"
    legacy_features = artifact_root / "features"
    current_features = artifact_root / "runs" / "current" / "features"
    legacy_features.mkdir(parents=True)
    current_features.mkdir(parents=True)
    pd.DataFrame({"protocol": ["modbus"], "packet_length": [60]}).to_parquet(
        legacy_features / "modbus.parquet", index=False
    )
    pd.DataFrame({"protocol": ["modbus"], "packet_length": [70]}).to_parquet(
        current_features / "modbus-observation.parquet", index=False
    )

    assert _discover_runs(artifact_root) == [current_features.parent]
