...
def _detectors(
    ...
    mlp_settings = {**models.get("mlp_autoencoder", {}), **overrides.get("mlp_autoencoder", {})}
    return {
        ...
        "mlp_autoencoder": MLPAutoencoder(
            **{
                **mlp_settings,
                "random_seed": int(
                    mlp_settings.get("random_seed", config["project"]["random_seed"])
                ),
            }
        ),
    }
...
def _anomaly_scores(model_name: str, model: Any, values: np.ndarray) -> np.ndarray:
    ...
    return raw if model_name in {"lstm_autoencoder", "mlp_autoencoder"} else -raw
...
def train_models(
    ...
    source_cap = config["models"].get("max_source_rows", 100_000)
    source_cap = int(source_cap) if source_cap is not None else None
    source = read_training_source(feature_path, maximum_rows=source_cap)
    ...
    with progress:
        ...
        for scope_name, protocols in scopes.items():
            ...
            _emit_progress(
                progress_callback,
                {
                    "event": "feature_matrix_preparing",
                    "scope": scope_name,
                    "protocols": protocols,
                    "source_rows": len(scope_source),
            ...
            prepared, _prepare_manifest, pipeline_path = prepare_features(
                ...
                progress_callback=lambda update, scope_name=scope_name, protocols=protocols: (
                    _emit_progress(
                        progress_callback,
                        {**update, "scope": scope_name, "protocols": protocols},
                    )
                ),
                source_frame=scope_source,
            )
            input_columns = [
            ...
            for model_name in candidate_names:
                ...
                if model_name in {"lstm_autoencoder", "mlp_autoencoder"}:
                    model.fit(
                        fit_values,
