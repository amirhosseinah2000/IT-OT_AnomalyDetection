...
def create_profile(
    ...
    # The selection itself, not creation time, determines the version.  This is
    # what lets an inference service prove that it received exactly the feature
    # contract used by training on another machine.
    protocol_scope = protocols or ["ssh", "dns", "http", "modbus", "s7comm"]
    version_seed = f"1.1.0|{name}|{','.join(cleaned)}|{','.join(protocol_scope)}|{description}"
...
def resolve_profile(profile: str | Path, config: dict[str, Any]) -> Path:
    ...
    if candidate.exists():
        return candidate
    matches = sorted(
        _profile_directory(config).glob(f"{profile}-*.json"),
