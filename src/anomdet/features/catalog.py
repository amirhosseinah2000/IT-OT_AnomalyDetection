...
def _feature(
    ...
    return FeatureDefinition(
        name, name.replace("_", " ").title(), protocols, category, description, cost
    )
...
FEATURE_CATALOG: tuple[FeatureDefinition, ...] = (
    ...
    _feature(
        "payload_entropy", ALL, "packet", "Shannon entropy of the transport payload.", "medium"
    ),
    ...
    _feature(
        ...
        ALL,
        "packet",
        "Whether the packet is travelling toward the recognised service endpoint.",
    ),
    _feature(
        "flow_duration", ALL, "flow", "Elapsed time from the first to current packet in a flow."
    ),
    _feature(
        "flow_total_packets", ALL, "flow", "Packets observed in the normalized bidirectional flow."
    ),
    _feature(
        "flow_total_bytes", ALL, "flow", "Bytes observed in the normalized bidirectional flow."
    ),
    _feature(
        "flow_byte_ratio",
        ALL,
        "flow",
        "Current directional bytes divided by reverse-direction bytes.",
    ),
    ...
    _feature(
        "inter_arrival_time", ALL, "timing", "Time since the previous packet in the same flow."
    ),
    ...
    _feature(
        ...
        ALL,
        "host_behavior",
        "Packets emitted by the source in the configured window.",
    ),
    _feature(
        "source_destination_count",
        ALL,
        "host_behavior",
        "Distinct destinations used by the source in the configured window.",
    ),
    _feature(
        "destination_entropy",
        ALL,
        "host_behavior",
        "Entropy of destinations contacted by the source.",
        "medium",
    ),
    _feature(
        "burstiness",
        ALL,
        "host_behavior",
        "Windowed source packet rate relative to global activity.",
    ),
    # SSH features.
    ...
    _feature(
        ...
        ("ssh",),
        "ssh_negotiation",
        "KEX algorithms exposed in an observable SSH KEXINIT.",
    ),
    _feature(
        "ssh_cipher_count",
        ("ssh",),
        "ssh_negotiation",
        "Number of advertised encryption algorithms.",
    ),
    _feature("ssh_mac_count", ("ssh",), "ssh_negotiation", "Number of advertised MAC algorithms."),
    _feature(
        "ssh_compression_count",
        ("ssh",),
        "ssh_negotiation",
        "Number of advertised compression algorithms.",
    ),
    _feature(
        "hassh",
        ("ssh",),
        "ssh_negotiation",
        "Hash of observable SSH KEXINIT algorithm lists.",
        "medium",
    ),
    _feature(
        "ssh_auth_method",
        ("ssh",),
        "ssh_authentication",
        "Authentication method if a protocol-aware parser can observe it.",
    ),
    _feature(
        "ssh_failed_auth_count",
        ("ssh",),
        "ssh_authentication",
        "Observable failed authentication events per flow.",
    ),
    _feature(
        "ssh_open_channel_count", ("ssh",), "ssh_session", "Observable SSH channel open events."
    ),
    _feature(
        "ssh_keepalive_interval",
        ("ssh",),
        "ssh_session",
        "Interval between observable keepalive packets.",
    ),
    # DNS features.
    ...
    _feature(
        "dns_qname_entropy", ("dns",), "dns_query", "Shannon entropy of the requested DNS name."
    ),
    _feature(
        "dns_label_count", ("dns",), "dns_query", "Number of labels in the requested DNS name."
    ),
    ...
    _feature(
        ...
        ("dns",),
        "dns_query",
        "Simple lexical score for domain-label naturalness.",
        "medium",
    ),
    ...
    _feature(
        "dns_recursion_available", ("dns",), "dns_response", "Recursion-available header flag."
    ),
    ...
    _feature(
        ...
        ("dns",),
        "dns_behavior",
        "NXDOMAIN ratio for the source during the capture.",
    ),
    _feature(
        "dns_query_repeat_count",
        ("dns",),
        "dns_behavior",
        "Repeat count for a source and DNS name.",
    ),
    # HTTP features.
    ...
    _feature(
        "http_query_parameter_count", ("http",), "http_request", "Number of URL query parameters."
    ),
    _feature(
        "http_suspicious_token_count",
        ("http",),
        "http_request",
        "SQLi, XSS, encoding, and traversal token count.",
    ),
    ...
    _feature(
        "http_error_ratio", ("http",), "http_behavior", "4xx/5xx response ratio for the source."
    ),
    _feature(
        "http_post_get_ratio",
        ("http",),
        "http_behavior",
        "POST-to-GET request ratio for the source.",
    ),
    _feature(
        "http_request_repeat_count",
        ("http",),
        "http_behavior",
        "Repeat count for source and request target.",
    ),
    # Modbus features.
    _feature(
        "modbus_function_code", ("modbus",), "modbus", "Modbus application PDU function code."
    ),
    _feature(
        "modbus_exception_code", ("modbus",), "modbus", "Modbus exception code, if applicable."
    ),
    _feature(
        "modbus_starting_address",
        ("modbus",),
        "modbus",
        "Requested register or coil start address.",
    ),
    ...
    _feature(
        "modbus_protocol_id_valid",
        ("modbus",),
        "modbus",
        "Whether MBAP protocol identifier equals zero.",
    ),
    _feature(
        "modbus_length_valid",
        ("modbus",),
        "modbus",
        "Whether MBAP length agrees with observed payload.",
    ),
    _feature(
        "modbus_function_category",
        ("modbus",),
        "modbus",
        "Read, write, diagnostic, or other function category.",
    ),
    _feature(
        "modbus_nonstandard_function",
        ("modbus",),
        "modbus",
        "Whether the function code is nonstandard or reserved.",
    ),
    _feature(
        "modbus_value", ("modbus",), "modbus", "First observable register value or written value."
    ),
    _feature(
        "modbus_exception_rate",
        ("modbus",),
        "modbus_behavior",
        "Exception ratio in the current capture.",
    ),
    _feature(
        "modbus_address_deviation",
        ("modbus",),
        "modbus_behavior",
        "Distance from the flow's median requested address.",
    ),
    _feature(
        "modbus_response_matched",
        ("modbus",),
        "modbus_behavior",
        "Whether a request/response pair with the same Modbus transaction identifier was observed.",
    ),
    # S7comm features.
    ...
    _feature(
        "s7_item_count", ("s7comm",), "s7comm", "Requested variable-item count where available."
    ),
    _feature(
        "s7_area_code", ("s7comm",), "s7comm", "Memory area code from a variable specification."
    ),
    _feature(
        "s7_db_number", ("s7comm",), "s7comm", "Data-block number from a variable specification."
    ),
    ...
    _feature(
        ...
        ("s7comm",),
        "s7comm_behavior",
        "Whether an opposite-direction PDU reference is observed.",
    ),
)
