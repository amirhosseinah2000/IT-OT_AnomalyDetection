...
from anomdet.core.io import read_table, write_json
...
def _discover_runs(artifact_root: Path) -> list[Path]:
    """Discover nested pipeline runs, using a direct table only as a legacy fallback."""
    ...
    for feature_dir in artifact_root.rglob("features"):
        if (
            feature_dir.is_dir()
            and feature_dir.parent != artifact_root
            and (
                any(feature_dir.glob("*.parquet"))
                or (feature_dir / "all-datasets.parquet").exists()
            )
        ):
...
def _read_training_progress(path: Path) -> dict[str, Any]:
    """Read a worker progress record without allowing a deleted old run to break the UI."""
    try:
        return _read_json(path) if path.exists() else {}
    except (OSError, json.JSONDecodeError):
        return {}
...
def _training_progress_for_run(run: Path) -> Path | None:
    """Restore the current run's newest monitor after navigation or browser refresh."""
    remembered = st.session_state.get("experiment_training_progress")
    if remembered:
        candidate = Path(str(remembered))
        try:
            is_current_run = candidate.is_relative_to(run)
        except ValueError:
            is_current_run = False
        if is_current_run and candidate.exists():
            return candidate
        st.session_state.pop("experiment_training_progress", None)
    candidates = _find_files(run / "experiments", "training-progress.json")
    return candidates[0] if candidates else None


def _training_event_rows(events: list[dict[str, Any]]) -> pd.DataFrame:
    """Keep the live log concise enough to read while preserving its diagnostic fields."""
    rows: list[dict[str, Any]] = []
    for event in events[-40:]:
        epoch = event.get("epoch")
        epochs = event.get("epochs")
        detail = event.get("feature_profile") or event.get("variant") or event.get("error")
        if detail is None and event.get("source_rows") is not None:
            detail = f"{int(event['source_rows']):,} source rows"
        if detail is None and event.get("prepared_rows") is not None:
            detail = f"{int(event['prepared_rows']):,} prepared rows"
        rows.append(
            {
                "time": event.get("at"),
                "elapsed (s)": event.get("elapsed_s"),
                "event": str(event.get("event", "update")).replace("_", " "),
                "scope": event.get("scope"),
                "model": event.get("model"),
                "epoch": f"{epoch}/{epochs}" if epoch is not None and epochs else epoch,
...
def _monitor_number(value: Any) -> str:
    """Format an optional numeric training value without making a missing loss look like zero."""
    try:
...
def _cancel_training_worker(progress_path: Path, progress: dict[str, Any]) -> str:
    """Stop only the tracked background worker, leaving the Streamlit server available."""
    try:
        os.kill(int(progress["pid"]), signal.SIGTERM)
    except (KeyError, TypeError, ValueError, ProcessLookupError) as error:
        return f"Could not stop this worker: {error}"
    progress["status"] = "cancelled"
    progress["cancelled_at"] = datetime.now(UTC).isoformat()
    events = progress.setdefault("events", [])
    if isinstance(events, list):
        events.append({"at": progress["cancelled_at"], "event": "worker_cancelled"})
    write_json(progress, progress_path)
    _load_json.clear()
    return "Training worker stopped. You can start a new experiment."


@st.fragment(run_every=2)
def _render_training_monitor(progress_path_text: str) -> None:
    """Poll and render background worker events independently of the rest of the dashboard."""
    progress_path = Path(progress_path_text)
    progress = _read_training_progress(progress_path)
    state = str(progress.get("status", "queued")).lower()
    events = progress.get("events", [])
    events = [item for item in events if isinstance(item, dict)] if isinstance(events, list) else []
    epoch_events = [item for item in events if item.get("event") == "epoch"]
    latest_epoch = epoch_events[-1] if epoch_events else None
    latest_event = events[-1] if events else {}
    model = str((latest_epoch or latest_event).get("model", "detector")).replace("_", " ").title()
    scope = str((latest_epoch or latest_event).get("scope", "selected protocol"))
    preparation_events = {
        "scope_started",
        "feature_matrix_preparing",
        "feature_source_loading",
        "feature_source_loaded",
        "feature_transform_started",
        "feature_transform_completed",
    }
    if state == "complete":
        label, status_state = "Training complete", "complete"
    elif state == "error":
        label, status_state = "Training failed", "error"
    elif state == "cancelled":
        label, status_state = "Training stopped", "error"
    elif latest_epoch:
        label = (
            f"Training {model} · {scope} · epoch {latest_epoch['epoch']}/{latest_epoch['epochs']}"
        )
        status_state = "running"
    elif latest_event.get("event") in preparation_events:
        source_rows = latest_event.get("source_rows") or latest_event.get("prepared_rows")
        row_text = f" · {int(source_rows):,} rows" if source_rows is not None else ""
        label, status_state = f"Preparing feature matrix · {scope}{row_text}", "running"
    elif state == "queued":
        label, status_state = "Training request queued", "running"
    else:
        label, status_state = f"Preparing {model} · {scope}", "running"
    st.status(label, state=status_state, expanded=state != "complete")

    progress_value = 0.0
    progress_text = "Waiting for the worker to prepare the feature matrix"
    if latest_epoch:
        epoch_count = max(int(latest_epoch.get("epochs", 1)), 1)
        progress_value = min(max(int(latest_epoch.get("epoch", 0)) / epoch_count, 0.0), 1.0)
        progress_text = f"{model} · {scope} · epoch {latest_epoch['epoch']}/{epoch_count}"
    elif state == "complete":
        progress_value, progress_text = 1.0, "All requested training and evaluation steps completed"
    st.progress(progress_value, text=progress_text)
    if state in {"queued", "running"}:
        if st.button(
            "Stop this training job",
            key=f"stop_training_{progress_path}",
            icon=":material/stop_circle:",
        ):
            message = _cancel_training_worker(progress_path, progress)
            if message.startswith("Could not"):
                st.error(message, icon=":material/error:")
            else:
                st.warning(message, icon=":material/stop_circle:")

    first, second, third, fourth = st.columns(4)
    first.metric(
        "Epoch",
        f"{latest_epoch['epoch']}/{latest_epoch['epochs']}" if latest_epoch else "Not started",
    )
    second.metric(
        "Train loss", _monitor_number(latest_epoch.get("train_loss") if latest_epoch else None)
    )
    third.metric(
        "Validation loss",
        _monitor_number(latest_epoch.get("validation_loss") if latest_epoch else None),
    )
    fourth.metric(
        "Best loss", _monitor_number(latest_epoch.get("best_loss") if latest_epoch else None)
    )
    if progress.get("error"):
        st.error(str(progress["error"]), icon=":material/error:")
    if state == "cancelled":
        st.warning(
            "This training job was stopped before completion.", icon=":material/stop_circle:"
        )
    result = progress.get("result")
    if isinstance(result, dict) and state == "complete":
        st.success(
            f"Completed {result.get('detector_runs', 0)} detector run(s). "
            "Open Results explorer > Model results to inspect them.",
            icon=":material/check_circle:",
        )
    st.caption("Live training log · refreshes every 2 seconds and remains available after refresh.")
    if events:
        st.dataframe(
            _training_event_rows(events), hide_index=True, height=290, use_container_width=True
        )
    else:
        st.caption("The background worker has not emitted its first event yet.")


def _launch_training_worker(request: dict[str, Any], output_root: Path) -> Path:
    """Persist one request and launch it outside Streamlit's synchronous rerun lifecycle."""
    output_root.mkdir(parents=True, exist_ok=True)
    request_path = output_root / "training-request.json"
    ...
    worker_python = (
        str(virtual_environment_python) if virtual_environment_python.exists() else sys.executable
    )
    with (output_root / "training-worker.log").open("a", encoding="utf-8") as worker_log:
        subprocess.Popen(
            [
                worker_python,
                "-m",
                "anomdet.dashboard.experiment_worker",
                "--request",
                str(request_path),
            ],
            cwd=Path.cwd(),
...
def _render_experiment_runner(
    ...
    existing_progress = _training_progress_for_run(run)
    if existing_progress is not None:
        _render_training_monitor(str(existing_progress))
    ...
    with st.form("experiment_runner"):
        ...
        with right:
            ...
            max_source_rows = st.number_input(
                "Maximum source rows for models",
                min_value=10_000,
                max_value=2_000_000,
                value=100_000,
                step=10_000,
                help=(
                    "A deterministic time-spread sample used by every selected model. "
                    "Keep this modest for interactive dashboard training."
                ),
            )
    ...
    try:
        current_state = (
            str(_read_training_progress(existing_progress).get("status", "")).lower()
            if existing_progress is not None
            else ""
        )
        if current_state in {"queued", "running"}:
            st.warning(
                "A training job is already running for this pipeline run. Its live log is above.",
        ...
        if run_sweep:
            ...
            if not variants:
                raise ValueError(
                    "Enter at least one hidden size and one sequence length for the sweep."
                )
            if len(variants) > 8:
                raise ValueError("Limit the LSTM sweep to eight parameter combinations per run.")
        settings = _config_for_dashboard("", artifact_root)
        settings["models"]["max_source_rows"] = int(max_source_rows)
        progress_path = _launch_training_worker(
            {
                "feature_path": str(feature_path),
                "config": settings,
                "strategy": strategy,
                "group": group,
                "profiles": [str(path) for path in selected_paths],
                "output_dir": str(output_root),
                "sweep_output_dir": str(output_root / "lstm-sweep"),
                "labels_path": str(selected_labels) if selected_labels else None,
                "candidates": models,
        ...
        st.session_state["experiment_training_progress"] = str(progress_path)
        st.toast(
            "Training started. The live monitor is updating now.", icon=":material/play_circle:"
        )
        st.rerun()
    except Exception as error:
        st.error(f"Could not start experiment: {error}", icon=":material/error:")
...
def main() -> None:
    """Render the streamlined artifact-first workbench.

    The legacy helper functions above remain import-compatible for existing
    automation/tests; the user-facing entrypoint intentionally stays small.
    """
    from anomdet.dashboard.workbench import render_workbench

    render_workbench()
    return

    # Historical experiment-studio implementation retained below for backwards
    # compatibility with its helper functions.
    _init_state()
    ...
    with st.sidebar:
        ...
        if st.button("Use all project runs", icon=":material/folder_open:"):
            st.session_state["artifact_root"] = "artifacts"
            st.rerun()
        ...
        if artifact_root.name != "artifacts" and (artifact_root / "features").is_dir():
            st.warning(
                "This path is a single run. Use 'Use all project runs' to select the newest "
                "pipeline output.",
        ...
        if runs:
            ...
            if sources:
                ...
                st.caption(
                    f"Loaded source: {_source_picker_label(active_protocol, active_feature_path)}"
                )
            st.caption(
